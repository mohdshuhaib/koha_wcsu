...
import RisalaHeader from '@/components/catalog/risala/RisalaHeader'
import RisalaFilters from '@/components/catalog/risala/RisalaFilters'
import RisalaCard from '@/components/catalog/risala/RisalaCard'
import RisalaDesktopRows from '@/components/catalog/risala/RisalaDesktopRows'
...
export default function RisalaCatalogPage() {
  const [items, setItems] = useState<Risala[]>([])
  ...
  const [mushrifFilter, setMushrifFilter] = useState('')
  ...
  const fetchItems = async () => {
    ...
    let query = supabase.from('risala').select('*')

    if (search.trim()) {
      const searchText = `%${search.trim()}%`
      query = query.or(
        `risala_name.ilike.${searchText},author.ilike.${searchText},mushrif.ilike.${searchText},department.ilike.${searchText},section_no.ilike.${searchText},barcode.ilike.${searchText}`
      )
    }
    ...
    if (departmentFilter.trim()) {
      query = query.ilike('department', `%${departmentFilter.trim()}%`)
    }

    if (mushrifFilter.trim()) {
      query = query.ilike('mushrif', `%${mushrifFilter.trim()}%`)
    }
    ...
    if (error) {
      console.error('Risala fetch error:', error)
      setItems([])
    } else {
      setItems((data as Risala[]) || [])
    }
  ...
  useEffect(() => {
    fetchItems()
  }, [search, languageFilter, statusFilter, departmentFilter, mushrifFilter, yearFilter])

  useEffect(() => {
    setPage(1)
  }, [search, languageFilter, statusFilter, departmentFilter, mushrifFilter, yearFilter, sortBy])
  ...
  const processedItems = useMemo(() => {
    ...
    cloned.sort((a, b) => {
      switch (sortBy) {
        case 'name_asc':
          return (a.risala_name || '').localeCompare(b.risala_name || '')
        case 'year':
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (error || !allItems) {
        throw new Error('Failed to fetch risala for export.')
      }

      const worksheet = XLSX.utils.json_to_sheet(allItems)
      const workbook = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Risala')

      XLSX.writeFile(workbook, 'risala_catalog.xlsx')
    } catch (err) {
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="relative z-10">
          <RisalaFilters
            ...
            mushrifFilter={mushrifFilter}
            setMushrifFilter={setMushrifFilter}
                    ...
                    <table className="min-w-full text-left text-sm">
                      <thead className="bg-secondary-light-black text-white">
                        <tr>
                          ...
                          <th className="px-4 py-4 font-semibold uppercase tracking-wider">Mushrif</th>
                          <th className="px-4 py-4 font-semibold uppercase tracking-wider">Department</th>
                          <th className="px-4 py-4 font-semibold uppercase tracking-wider">Year</th>
                          <th className="px-4 py-4 font-semibold uppercase tracking-wider">Section No</th>
