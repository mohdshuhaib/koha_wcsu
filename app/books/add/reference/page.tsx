...
export default function AddReferencePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('reference').insert([dataToInsert])
    if (error) {
      setError(`Failed to add reference: ${error.message}`)
    } else {
      setSuccess(`Successfully added "${formData.reference_name}" to the catalog!`)
      setFormData({
        reference_name: '', author: '', language: '', price: '', publication: '',
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              <div className="md:col-span-2">
                <label htmlFor="reference_name" className="block text-sm font-semibold text-text-grey mb-1">Reference Name</label>
                <input id="reference_name" name="reference_name" value={formData.reference_name} onChange={handleChange} required className="w-full p-3 rounded-lg bg-primary-grey border border-primary-dark-grey text-text-grey placeholder-text-grey focus:outline-none focus:ring-2 focus:ring-dark-green" />
              </div>
              ...
              <div>
                ...
                <input id="price" name="price" type="number" min="0" step="0.01" value={formData.price} onChange={handleChange} className="w-full p-3 rounded-lg bg-primary-grey border border-primary-dark-grey text-text-grey placeholder-text-grey focus:outline-none focus:ring-2 focus:ring-dark-green" />
              </div>
              ...
              <div>
                ...
                <input id="publication" name="publication" value={formData.publication} onChange={handleChange} className="w-full p-3 rounded-lg bg-primary-grey border border-primary-dark-grey text-text-grey placeholder-text-grey focus:outline-none focus:ring-2 focus:ring-dark-green" />
              </div>
