...
export default function BulkUploadReferencePage() {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const { error } = await supabase.from('reference').insert(references)

      if (error) {
        throw new Error(`Upload failed: ${error.message}`)
      } else {
        setUploadResult({ type: 'success', message: `${references.length} references were uploaded successfully!` })
  ...
  const handleDownloadTemplate = () => {
    const headers = [['reference_name', 'author', 'language', 'price', 'publication', 'barcode']];
    const ws = XLSX.utils.aoa_to_sheet(headers);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'References');
    XLSX.writeFile(wb, 'reference_template.xlsx');
  }
        ...
        <div className="bg-secondary-white p-6 md:p-8 rounded-2xl shadow-xl border border-primary-dark-grey">
          <div className="text-center border-b border-primary-dark-grey pb-6 mb-6">
            <h1 className="text-2xl font-bold text-heading-text-black uppercase font-heading tracking-wider">
              Bulk Upload Reference Books
            </h1>
            <p className="text-text-grey mt-1">Upload an Excel (.xlsx) file to add multiple reference books to the catalog at once.</p>
          </div>
            ...
            <div className="bg-primary-grey p-6 rounded-lg border border-primary-dark-grey">
              ...
              <div className="space-y-3 text-sm text-text-grey">
                ...
                <div className="flex flex-wrap gap-2">
                  {['reference_name', 'language', 'barcode'].map(col => (
                ...
                <div className="flex flex-wrap gap-2">
                  {['author', 'price', 'publication'].map(col => (
                    <code key={col} className="px-2 py-1 bg-gray-300 text-heading-text-black rounded text-xs font-semibold">{col}</code>
                  ))}
                </div>
